...
paths. For the code to manage those addons, see odoo.microkernel.modules.

Addons are made available under `odoo.addons` after
odoo.conf.config.parse_config() is called (so that the addons paths are
known).

This module also conveniently reexports some symbols from odoo.microkernel.modules.
