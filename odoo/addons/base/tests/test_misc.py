...
from odoo.technology.utils import date_utils, file_open, file_path, merge_sequences
...
class TestFormatLangDate(TransactionCase):
    def test_00_accepted_types(self):
        ...
        self.assertEqual(microkernel_utils.format_date(self.env, date_datetime), '01/31/2017')
        self.assertEqual(microkernel_utils.format_date(self.env, date_date), '01/31/2017')
        self.assertEqual(microkernel_utils.format_date(self.env, date_str), '01/31/2017')
        self.assertEqual(microkernel_utils.format_date(self.env, ''), '')
        self.assertEqual(microkernel_utils.format_date(self.env, False), '')
        self.assertEqual(microkernel_utils.format_date(self.env, None), '')

        self.assertEqual(microkernel_utils.format_datetime(self.env, date_datetime, dt_format=medium), 'Jan 31, 2017, 1:00:00 PM')
        self.assertEqual(microkernel_utils.format_datetime(self.env, datetime_str, dt_format=medium), 'Jan 31, 2017, 1:00:00 PM')
        self.assertEqual(microkernel_utils.format_datetime(self.env, '', dt_format=medium), '')
        self.assertEqual(microkernel_utils.format_datetime(self.env, False, dt_format=medium), '')
        self.assertEqual(microkernel_utils.format_datetime(self.env, None, dt_format=medium), '')

        self.assertEqual(microkernel_utils.format_time(self.env, time_part, time_format=t_medium), '4:30:22 PM')
        self.assertEqual(microkernel_utils.format_time(self.env, '', time_format=t_medium), '')
        self.assertEqual(microkernel_utils.format_time(self.env, False, time_format=t_medium), '')
        self.assertEqual(microkernel_utils.format_time(self.env, None, time_format=t_medium), '')
    ...
    def test_01_code_and_format(self):
        ...
        self.assertEqual(microkernel_utils.format_date(lang.with_context(lang='fr_FR').env, date_str), '31/01/2017')
        self.assertEqual(microkernel_utils.format_date(lang.env, date_str, lang_code='fr_FR'), '31/01/2017')
        self.assertEqual(microkernel_utils.format_date(lang.env, date_str, date_format='MMM d, y'), 'Jan 31, 2017')

        # Change 2 parameters
        self.assertEqual(microkernel_utils.format_date(lang.with_context(lang='zh_CN').env, date_str, lang_code='fr_FR'), '31/01/2017')
        self.assertEqual(microkernel_utils.format_date(lang.with_context(lang='zh_CN').env, date_str, date_format='MMM d, y'), u'1\u6708 31, 2017')
        self.assertEqual(microkernel_utils.format_date(lang.env, date_str, lang_code='fr_FR', date_format='MMM d, y'), 'janv. 31, 2017')

        # Change 3 parameters
        self.assertEqual(microkernel_utils.format_date(lang.with_context(lang='zh_CN').env, date_str, lang_code='en_US', date_format='MMM d, y'), 'Jan 31, 2017')

        # -- test `datetime`
        datetime_str = '2017-01-31 10:33:00'
        # Change languages and timezones
        datetime_us_str = microkernel_utils.format_datetime(lang.with_context(lang='en_US').env, datetime_str, tz='Europe/Brussels')
        self.assertNotEqual(microkernel_utils.format_datetime(lang.with_context(lang='fr_FR').env, datetime_str, tz='Europe/Brussels'), datetime_us_str)
        self.assertNotEqual(microkernel_utils.format_datetime(lang.with_context(lang='zh_CN').env, datetime_str, tz='America/New_York'), datetime_us_str)

        # Change language, timezone and format
        self.assertEqual(microkernel_utils.format_datetime(lang.with_context(lang='fr_FR').env, datetime_str, tz='America/New_York', dt_format='dd/MM/YYYY HH:mm'), '31/01/2017 05:33')
        self.assertEqual(microkernel_utils.format_datetime(lang.with_context(lang='en_US').env, datetime_str, tz='Europe/Brussels', dt_format='MMM d, y'), 'Jan 31, 2017')

        # Check given `lang_code` overwites context lang
        fmt_fr = 'dd MMMM YYYY à HH:mm:ss Z'
        fmt_us = "MMMM dd, YYYY 'at' hh:mm:ss a Z"
        self.assertEqual(microkernel_utils.format_datetime(lang.env, datetime_str, tz='Europe/Brussels', dt_format=fmt_fr, lang_code='fr_FR'), '31 janvier 2017 à 11:33:00 +0100')
        self.assertEqual(microkernel_utils.format_datetime(lang.with_context(lang='zh_CN').env, datetime_str, tz='Europe/Brussels', dt_format=fmt_us, lang_code='en_US'), 'January 31, 2017 at 11:33:00 AM +0100')

        # -- test `time`
        time_part = datetime.time(16, 30, 22)
        time_part_tz = datetime.time(16, 30, 22, tzinfo=pytz.timezone('America/New_York'))  # 4:30 PM timezoned

        self.assertEqual(microkernel_utils.format_time(lang.with_context(lang='fr_FR').env, time_part, time_format='HH:mm:ss'), '16:30:22')
        self.assertEqual(microkernel_utils.format_time(lang.with_context(lang='zh_CN').env, time_part, time_format="ah:m:ss"), '\u4e0b\u53484:30:22')

        # Check format in different languages
        self.assertEqual(microkernel_utils.format_time(lang.with_context(lang='fr_FR').env, time_part, time_format='HH:mm'), '16:30')
        self.assertEqual(microkernel_utils.format_time(lang.with_context(lang='zh_CN').env, time_part, time_format='ah:mm'), '\u4e0b\u53484:30')

        # Check timezoned time part
        self.assertEqual(microkernel_utils.format_time(lang.with_context(lang='fr_FR').env, time_part_tz, time_format='HH:mm:ss Z'), '16:30:22 -0504')
        self.assertEqual(microkernel_utils.format_time(lang.with_context(lang='zh_CN').env, time_part_tz, time_format='zzzz ah:mm:ss'), '\u5317\u7f8e\u4e1c\u90e8\u6807\u51c6\u65f6\u95f4\u0020\u4e0b\u53484:30:22')

        #Check timezone conversion in format_time
        self.assertEqual(microkernel_utils.format_time(lang.with_context(lang='fr_FR').env, datetime_str, 'Europe/Brussels', time_format='HH:mm:ss Z'), '11:33:00 +0100')
        self.assertEqual(microkernel_utils.format_time(lang.with_context(lang='fr_FR').env, datetime_str, 'America/New_York', time_format='HH:mm:ss Z'), '05:33:00 -0500')

        # Check given `lang_code` overwites context lang
        self.assertEqual(microkernel_utils.format_time(lang.with_context(lang='fr_FR').env, time_part, time_format='ah:mm', lang_code='zh_CN'), '\u4e0b\u53484:30')
        self.assertEqual(microkernel_utils.format_time(lang.with_context(lang='zh_CN').env, time_part, time_format='ah:mm', lang_code='fr_FR'), 'PM4:30')
    ...
    def test_02_tz(self):
        ...
        self.assertEqual(microkernel_utils.format_date(self.env, date_datetime), '01/01/2017')

        # Force London timezone
        date_datetime = date_datetime.replace(tzinfo=pytz.UTC)
        self.assertEqual(microkernel_utils.format_date(self.env, date_datetime), '12/31/2016', "User's tz must be ignored when tz is specifed in datetime object")


class TestCallbacks(BaseCase):
    def test_callback(self):
        log = []
        callbacks = tech_utils.Callbacks()
...
class TestFormatLang(TransactionCase):
    def test_value_and_digits(self):
        self.assertEqual(i18n.formatLang(self.env, 100.23, digits=1), '100.2')
        self.assertEqual(i18n.formatLang(self.env, 100.23, digits=3), '100.230')

        self.assertEqual(i18n.formatLang(self.env, ''), '', 'If value is an empty string, it should return an empty string (not 0)')

        self.assertEqual(i18n.formatLang(self.env, 100), '100.00', 'If digits is None (default value), it should default to 2')

        # Default rounding is 'HALF_EVEN'
        self.assertEqual(i18n.formatLang(self.env, 100.205), '100.20')
        self.assertEqual(i18n.formatLang(self.env, 100.215), '100.22')
    ...
    def test_grouping(self):
        ...
        self.assertEqual(i18n.formatLang(self.env['res.lang'].with_context(lang='fLT').env, 1000000000, grouping=True), '10000?00?000!00')
        self.assertEqual(i18n.formatLang(self.env['res.lang'].with_context(lang='fLT').env, 1000000000, grouping=False), '1000000000.00')
    ...
    def test_decimal_precision(self):
        ...
        self.assertEqual(i18n.formatLang(self.env, 100, dp=decimal_precision.name), '100.000')
    ...
    def test_currency_object(self):
        ...
        self.assertEqual(i18n.formatLang(self.env, 100, currency_obj=currency_object), '100.0%sfL' % u'\N{NO-BREAK SPACE}')

        currency_object.write({'position': 'before'})

        self.assertEqual(i18n.formatLang(self.env, 100, currency_obj=currency_object), 'fL%s100.0' % u'\N{NO-BREAK SPACE}')
    ...
    def test_decimal_precision_and_currency_object(self):
        ...
        self.assertEqual(i18n.formatLang(self.env, 100, dp=decimal_precision.name, currency_obj=currency_object), '100.000%sfL' % u'\N{NO-BREAK SPACE}')

    def test_rounding_method(self):
        self.assertEqual(i18n.formatLang(self.env, 100.205), '100.20')  # Default is 'HALF-EVEN'
        self.assertEqual(i18n.formatLang(self.env, 100.215), '100.22')  # Default is 'HALF-EVEN'

        self.assertEqual(i18n.formatLang(self.env, 100.205, rounding_method='HALF-UP'), '100.21')
        self.assertEqual(i18n.formatLang(self.env, 100.215, rounding_method='HALF-UP'), '100.22')

        self.assertEqual(i18n.formatLang(self.env, 100.205, rounding_method='HALF-DOWN'), '100.20')
        self.assertEqual(i18n.formatLang(self.env, 100.215, rounding_method='HALF-DOWN'), '100.21')

    def test_rounding_unit(self):
        self.assertEqual(i18n.formatLang(self.env, 1000000.00), '1,000,000.00')
        self.assertEqual(i18n.formatLang(self.env, 1000000.00, rounding_unit='units'), '1,000,000')
        self.assertEqual(i18n.formatLang(self.env, 1000000.00, rounding_unit='thousands'), '1,000')
        self.assertEqual(i18n.formatLang(self.env, 1000000.00, rounding_unit='lakhs'), '10')
        self.assertEqual(i18n.formatLang(self.env, 1000000.00, rounding_unit="millions"), '1')

    def test_rounding_method_and_rounding_unit(self):
        self.assertEqual(i18n.formatLang(self.env, 1822060000, rounding_method='HALF-UP', rounding_unit='lakhs'), '18,221')
        self.assertEqual(i18n.formatLang(self.env, 1822050000, rounding_method='HALF-UP', rounding_unit='lakhs'), '18,221')
        self.assertEqual(i18n.formatLang(self.env, 1822049900, rounding_method='HALF-UP', rounding_unit='lakhs'), '18,220')
...
class TestMiscToken(TransactionCase):
    ...
    def test_expired_token(self):
        ...
        token = tech_utils.hash_sign(self.env, 'test', payload, expiration=expiration)
        self.assertIsNone(tech_utils.verify_hash_signed(self.env, 'test', token))

    def test_long_payload(self):
        payload = {'test': True, 'value':123456, 'some_string': 'hello', 'some_dict': {'name': 'New Dict'}}
        token = tech_utils.hash_sign(self.env, 'test', payload, expiration_hours=24)
        self.assertEqual(tech_utils.verify_hash_signed(self.env, 'test', token), payload)

    def test_None_payload(self):
        with self.assertRaises(Exception):
            tech_utils.hash_sign(self.env, 'test', None, expiration_hours=24)
    ...
    def test_modified_payload(self):
        payload = ["str1", "str2", "str3", 4, 5]
        token = base64.urlsafe_b64decode(tech_utils.hash_sign(self.env, 'test', payload, expiration_hours=24) + '===')
